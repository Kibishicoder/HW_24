...
from project_data.config import COMMANDS
...
class Commands:
    ...
    def __post_init__(self):
        ...
        if self.cmd1 not in COMMANDS:
            raise ValidationError(f'Command given as cmd1 ({self.cmd1}) is not a valid command. '
                                  f'Only {", ".join(COMMANDS)} allowed')
        if self.cmd2 not in COMMANDS:
            raise ValidationError(f'Command given as cmd2 ({self.cmd2}) is not a valid command. '
                                  f'Only {", ".join(COMMANDS)} allowed')
